      ...
      <CardContent className="flex flex-col gap-3 p-5">
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            <div className="mt-0.5 line-clamp-2 text-sm text-gray-500">
              {tree.description || "کوئی تفصیل نہیں"}
            </div>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-gray-500">
          <span className="inline-flex items-center gap-1">
            ...
            {tree.memberCount} ممبرز
          </span>
          <span className="inline-flex items-center gap-1">
            <GitFork className="h-3.5 w-3.5" />
            {tree.generationCount} نسلیں
          </span>
          <span className="inline-flex items-center gap-1">
            <VisIcon className="h-3.5 w-3.5" />
            {vis.label} · {vis.urdu}
          </span>
          {!isOwner && ownerName && <span className="text-gray-400">از: {ownerName}</span>}
        </div>
        ...
        <div className="mt-1 flex items-center gap-2 border-t pt-3">
          <Button size="sm" className="flex-1 bg-emerald-600 hover:bg-emerald-700" asChild>
            <Link href={`/tree/${tree.id}`}>کھولیں</Link>
          </Button>
          {isOwner && onDelete && (
            <Button
              size="sm"
